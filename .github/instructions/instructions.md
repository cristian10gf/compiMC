...
Reglas de comportamiento para el agente — Arquitectura y estilo (añadir al conjunto de instrucciones):

1. Priorizar la reutilización de componentes existentes
   - Antes de crear cualquier componente nuevo, buscar en el repositorio por componentes y utilidades ya implementadas y preferir su reutilización o extensión.
   - Reusar variantes y props en lugar de duplicar UI o lógica; crear componentes genéricos y pequeños cuando haga falta.
   - Mantener la convención de nombres del proyecto (kebab-case para archivos) y respetar las utilidades y tokens existentes.

2. Estructura correcta en React / Next.js y control de renders
   - Favorecer Server Components para la mayor parte de la UI; usar `use client` únicamente en componentes interactivos y, preferiblemente, lo más abajo posible en el árbol de componentes (solo los nodos hoja que requieren interactividad deben ser client).
   - Minimizar re-renderizados: aplicar `React.memo`, `useMemo`, `useCallback` y patrones de prop-types inmutables; evitar pasar objetos o funciones inline como props salvo cuando sea inevitables.
   - Evitar fetching dentro de `useEffect` para datos que puedan venir de RSC o Server Actions; cuando haga falta fetching en cliente, preferir SWR u otra librería especificada por el proyecto y centralizar la lógica de caché.
   - Diseñar la jerarquía de estado para que el estado que provoca re-render global esté lo más alto necesario pero sin provocar renders innecesarios en subárboles. Evitar estados locales duplicados y preferir pasar datos via props o context soltado en niveles adecuados.

3. Política de componentes "client" mínimos
   - Solo marcar con `'use client'` los componentes que realmente necesitan APIs del cliente (event handlers, hooks de estado locales, acceso al `window`, etc.).
   - Todos los padres que no requieran interacción deben permanecer como Server Components; los componentes client deben ser nodos hoja y lo más pequeños posible.
   - Documentar en el comentario del archivo por qué el componente es `client` cuando no sea obvio.

4. Evitar el uso de emojis
   - Prohibido el uso de emojis en texto UI, mensajes de sistema, commits de ejemplo, o comentarios visibles al usuario. Usar iconografía apropiada del proyecto (SVGs o componentes de iconos) según las guías de diseño.

5. Seguir el estilo general preestablecido del proyecto
   - Respetar las pautas de diseño y código del repositorio: convenciones de nombre, sistemas de tokens, clases Tailwind y pautas de accesibilidad, separación de componentes en archivos, y patterns ya adoptados (p. ej. uso de shadcn/ui si está presente).
   - No sobrescribir archivos globales por defecto salvo que el cambio sea explícitamente solicitado.

6. Consultar siempre la documentación oficial y actualizada de librerías
   - Antes de usar una API de terceros o una nueva versión de una dependencia, el agente debe consultar la documentación oficial más reciente en línea y preferirla frente a ejemplos no oficiales.
   - Registrar la fuente (URL de la documentación y fecha consultada) en el comentario o en el ticket/PR asociado.
   - Si existen cambios relevantes en la API entre versiones, proponer la alternativa compatible con la versión fijada en el proyecto y documentar la decisión.

7. Comportamiento operativo
   - Buscar en el repo (y revisar todas las coincidencias relevantes) antes de agregar, renombrar o modificar componentes.
   - Cuando se realicen cambios, incluir un breve postamble de 2–4 frases que resuma qué se cambió y por qué, y listar las fuentes de documentación consultadas si aplica.

Observación: estas reglas deben aplicarse siempre en conjunto con las demás guías del proyecto (naming, Tailwind, uso de SWR, reglas de AI SDK y demás). Para referencia de estilo y convenciones del workspace, consulte las pautas base del proyecto. :contentReference[oaicite:0]{index=0}
